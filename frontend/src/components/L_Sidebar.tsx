...
const L_Sidebar:React.FC = () => {
    ...
    return (
        <div className="fixed flex flex-col items-center text-center w-25 z-40">
            <button className='flex flex-col border-2  items-center justify-center rounded-lg m-4 p-2 mt-50 cursor-pointer border-green-600 bg-green-500 hover:bg-green-600' onClick={() => navigate('/grammar-checker')}>
                <img className='flex w-9 h-9 ' src={G_check_img} alt="Paraphrase_picture"/>
                <p className='text-[12px] ml-2 mr-2'>Grammar Checker</p>
            </button>
            <button className='flex flex-col border-2 items-center justify-center rounded-lg m-4 p-2 cursor-pointer border-green-600 bg-green-500 hover:bg-green-600'onClick={() => navigate('/similar')}>
